...
class BuilderBase {
  ...
  CSRGraph<NodeID_, DestID_, invert> RandOrder (
      const CSRGraph<NodeID_, DestID_, invert> &g, pvector<NodeID_> &new_ids, bool createOnlyDegList, bool createBothCSRs) 
    {
      ...
      if (g.directed() == true) {
        ...
        #pragma omp parallel
        {
          std::mt19937 rng(kRandSeed + omp_get_thread_num());
          std::uniform_int_distribution<NodeID_> udist(0, g.num_nodes()-1);
  ...
  */
  static void makeOffsetMatrix(const CSRGraph<NodeID_, DestID_, invert> &g, 
                                      pvector<uint8_t> &offsetMatrix, int numVtxPerLine, bool traverseCSR = true)
    {
      assert(g.directed() == true); //TODO: Add support for undirected CSR
      Timer tm;
      #if 0
      ...
      for (NodeID_ c = 0; c < numCacheLines; ++c)
      {
        ...
        for (NodeID_ e = numEpochs - 2; e >= 0; --e)
        {
          if (lastRef[(c * numEpochs) + e] != -1)
          {
            ...
            compressedOffsets[(c * numEpochs) + e] = static_cast<uint8_t>(subEpochDist / subEpochSz);
            compressedOffsets[(c * numEpochs) + e] &= andMask; 
          }
          else
          {
            if ((compressedOffsets[(c * numEpochs) + e + 1] & orMask) != 0)
            {
              ...
              compressedOffsets[(c * numEpochs) + e] = compressedOffsets[(c * numEpochs) + e + 1];
              compressedOffsets[(c * numEpochs) + e] = std::min(compressedOffsets[(c * numEpochs) + e], maxReref);
            }
            else
            {
