...
NOTE: shared_size needs to be selected carefully! If we put more entries 
into the queue then we run into buffer overflow errors
*/
...
class QueueBuffer {
 ...
 public:
  ...
  void flush() {
    ...
    assert((copy_start + in) <= sq.total_size);
